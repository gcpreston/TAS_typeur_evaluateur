open Typeur_evaluateur

(* ***EXEMPLES*** *)
let ex_id : Common.pterm = Abs ("x", Var "x")
let inf_ex_id : string = Typeur.print_inference ex_id
let ex_k : Common.pterm = Abs ("x", Abs ("y", Var "x"))
let inf_ex_k : string = Typeur.print_inference ex_k

let ex_s : Common.pterm =
  Abs
    ( "x",
      Abs ("y", Abs ("z", App (App (Var "x", Var "z"), App (Var "y", Var "z"))))
    )

let inf_ex_s : string = Typeur.print_inference ex_s
let ex_nat1 : Common.pterm = App (Abs ("x", Add (Var "x", N 1)), N 3)
let inf_ex_nat1 : string = Typeur.print_inference ex_nat1
let ex_nat2 : Common.pterm = Abs ("x", Add (Var "x", Var "x"))
let inf_ex_nat2 : string = Typeur.print_inference ex_nat2

let ex_omega : Common.pterm =
  App (Abs ("x", App (Var "x", Var "x")), Abs ("y", App (Var "y", Var "y")))

let inf_ex_omega : string = Typeur.print_inference ex_omega
let ex_nat3 : Common.pterm = App (ex_nat2, ex_id)
let inf_ex_nat3 : string = Typeur.print_inference ex_nat3
let ex_lst1 : Common.pterm = EmptyList
let inf_ex_lst1 : string = Typeur.print_inference ex_lst1
let ex_lst2 : Common.pterm = Cons (N 5, EmptyList)
let inf_ex_lst2 : string = Typeur.print_inference ex_lst2
let ex_hd : Common.pterm = Head (Cons (N 5, EmptyList))
let inf_ex_hd : string = Typeur.print_inference ex_hd
let ex_tl : Common.pterm = Tail (Cons (N 5, EmptyList))
let inf_ex_tl : string = Typeur.print_inference ex_tl
let ex_ifzero : Common.pterm = IfZero (N 0, N 5, EmptyList)
let inf_ex_ifzero : string = Typeur.print_inference ex_ifzero
let ex_ifempty : Common.pterm = IfEmpty (EmptyList, N 5, N 10)
let inf_ex_ifempty : string = Typeur.print_inference ex_ifempty
let ex_let : Common.pterm = Let ("x", N 5, Add (Var "x", N 4))
let inf_ex_let : string = Typeur.print_inference ex_let

let ex_let_poly : Common.pterm =
  Let
    ( "id",
      Abs ("x", Var "x"),
      Let ("_", App (Var "id", N 0), App (Var "id", EmptyList)) )

let inf_ex_let_poly : string = Typeur.print_inference ex_let_poly

let ex_let_fail : Common.pterm =
  Let
    ( "id",
      Abs ("x", Var "x"),
      IfEmpty (EmptyList, App (Var "id", N 5), App (Var "id", EmptyList)) )

let inf_ex_let_fail : string = Typeur.print_inference ex_let_fail
let ex_ref : Common.pterm = Let ("x", Ref EmptyList, Cons (N 1, Deref (Var "x")))
let inf_ex_ref : string = Typeur.print_inference ex_ref

let ex_assign : Common.pterm =
  Let
    ( "l",
      Ref EmptyList,
      Let
        ( "_",
          Assign (Var "l", Cons (Abs ("x", Var "x"), EmptyList)),
          Add (Head (Deref (Var "l")), N 2) ) )

let inf_ex_assign : string = Typeur.print_inference ex_assign

let ex_fix : Common.pterm =
  Fix
    ( "phi",
      Abs ("n", IfZero (Var "n", N 123, App (Var "phi", Sub (Var "n", N 1)))) )

let inf_ex_fix : string = Typeur.print_inference ex_fix

open Typeur_evaluateur

(* ***EXEMPLES*** *)
let ex_id : Common.pterm = Abs ("x", Var "x")
let conv_ex_id = Evaluateur.alpha_convert ex_id
let ex_k : Common.pterm = Abs ("x", Abs ("y", Var "x"))
let conv_ex_k = Evaluateur.alpha_convert ex_k

let ex_s : Common.pterm =
  Abs
    ( "x",
      Abs ("y", Abs ("z", App (App (Var "x", Var "z"), App (Var "y", Var "z"))))
    )

let conv_ex_s = Evaluateur.alpha_convert ex_s

let ex_relink : Common.pterm =
  Abs ("x", Abs ("y", App (Var "x", Abs ("x", App (Var "x", Var "y")))))

let conv_ex_relink = Evaluateur.alpha_convert ex_relink
let ex_sub : Common.pterm = App (Var "x", Abs ("x", Var "x"))

let sub_ex_sub =
  Evaluateur.substitue_var ex_sub "x" (Abs ("y", App (Var "y", Var "y")))

let ex_id_k : Common.pterm = App (ex_id, ex_k)
let eval_ex_id_k = Evaluateur.eval ex_id_k

(* let ex_add : Common.pterm = Abs ("n", Abs ("m", Abs ("f", Abs ("e", App(Var "n", App (Var "f", App (Var "m", App (Var "f", Var "e"))))))))
let ex_mult : Common.pterm = Abs ("n", Abs ("m", Abs ("f", Abs ("e", App(App (Var "n", App (Var "m", Var "f")), Var "e")))))
let ex_2 : Common.pterm = Abs ("f", Abs ("e", App (Var "f", App (Var "f", Var "e"))))
let ex_3 : Common.pterm = Abs ("f", Abs ("e", App (Var "f", App (Var "f", App (Var "f", Var "e")))))
let ex_add_2_3 : Common.pterm = App (App (ex_add, ex_2), ex_3)
let eval_ex_add_2_3 = Evaluateur.eval ex_add_2_3 *)
(* let ex_delta : Common.pterm = Abs ("x", App (Var "x", Var "x"))
let ex_delta_delta : Common.pterm = App (ex_delta, ex_delta)
let eval_ex_delta_delta = Evaluateur.eval ex_delta_delta *)
let ex_id_id_5 : Common.pterm = App (ex_id, App (ex_id, N 5))
let eval_ex_id_id_5 = Evaluateur.eval ex_id_id_5
let ex_let : Common.pterm = Let ("x", Add (N 1, N 2), Add (Var "x", N 10))
let eval_ex_let = Evaluateur.eval ex_let

let ex_fix : Common.pterm =
  App
    ( Fix
        ( "phi",
          Abs ("n", IfZero (Var "n", N 123, App (Var "phi", Sub (Var "n", N 1))))
        ),
      N 3 )

let eval_ex_fix = Evaluateur.eval ex_fix
let ex_ref : Common.pterm = Let ("r", Ref EmptyList, Deref (Var "r"))
let eval_ex_ref = Evaluateur.eval ex_ref

let ex_assign : Common.pterm =
  Let
    ( "l",
      Ref EmptyList,
      Let
        ( "_",
          Assign (Var "l", Cons (Abs ("x", Var "x"), EmptyList)),
          Add (Head (Deref (Var "l")), N 2) ) )

let eval_ex_assign = Evaluateur.eval ex_assign

let main () =
  print_endline "Typage";
  print_endline "======================";
  print_endline inf_ex_id;
  print_endline "======================";
  print_endline inf_ex_k;
  print_endline "======================";
  print_endline inf_ex_s;
  print_endline "======================";
  print_endline inf_ex_omega;
  print_endline "======================";
  print_endline inf_ex_nat1;
  print_endline "======================";
  print_endline inf_ex_nat2;
  print_endline "======================";
  print_endline inf_ex_nat3;
  print_endline "======================";
  print_endline inf_ex_lst1;
  print_endline "======================";
  print_endline inf_ex_lst2;
  print_endline "======================";
  print_endline inf_ex_hd;
  print_endline "======================";
  print_endline inf_ex_tl;
  print_endline "======================";
  print_endline inf_ex_ifzero;
  print_endline "======================";
  print_endline inf_ex_ifempty;
  print_endline "======================";
  print_endline inf_ex_let;
  print_endline "======================";
  print_endline inf_ex_let_poly;
  print_endline "======================";
  print_endline inf_ex_let_fail;
  print_endline "======================";
  print_endline inf_ex_ref;
  print_endline "======================";
  print_endline inf_ex_assign;
  print_endline "======================";
  print_endline inf_ex_fix;
  print_endline "";
  print_endline "Alpha-conversion";
  print_endline "======================";
  print_endline (Common.print_term conv_ex_id);
  print_endline "======================";
  print_endline (Common.print_term conv_ex_k);
  print_endline "======================";
  print_endline (Common.print_term conv_ex_s);
  print_endline "======================";
  print_endline (Common.print_term conv_ex_relink);
  (* print_endline "======================";
  print_endline (Common.print_term (Evaluateur.alpha_convert ex_ref)); *)
  print_endline "";
  print_endline "Substitution";
  print_endline "======================";
  print_endline (Common.print_term sub_ex_sub);
  print_endline "";
  print_endline "Evaluation";
  print_endline "======================";
  (* print_endline (Common.print_term eval_ex_add_2_3); *)
  print_endline (Common.print_term eval_ex_id_k);
  print_endline "======================";
  print_endline (Common.print_term eval_ex_id_id_5);
  print_endline "======================";
  print_endline (Common.print_term eval_ex_let);
  print_endline "======================";
  print_endline (Common.print_term eval_ex_fix);
  print_endline "======================";
  print_endline (Common.print_term eval_ex_ref);
  print_endline "======================";
  print_endline (Common.print_term eval_ex_assign)

let _ = main ()
