...
let rec substitue_type (t : ptype) (v : string) (t0 : ptype) : ptype =
  ...
  | ArrowType (t1, t2) ->
      ArrowType (substitue_type t1 v t0, substitue_type t2 v t0)
  | NatType -> NatType
...
let rec variables_libres_helper (t : ptype) (acc : string list) : string list =
  ...
  | VarType v -> v :: acc
  | ArrowType (t1, t2) ->
      variables_libres_helper t1 acc @ variables_libres_helper t2 acc
...
let (* rec *) rembobine (e : equa_zip) =
  match e with [], _ -> e | c :: e1, e2 -> (e1, c :: e2)
...
let rec unification (e : equa_zip) (but : string) : ptype =
  ...
  | e1, (VarType v1, VarType v2) :: e2 ->
      unification (substitue_type_zip (rembobine (e1, e2)) v2 (VarType v1)) but
  ...
  | e1, (VarType v1, t2) :: e2 ->
      ...
      else unification (substitue_type_zip (rembobine (e1, e2)) v1 t2) but
  ...
  | e1, (t1, VarType v2) :: e2 ->
      ...
      else unification (substitue_type_zip (rembobine (e1, e2)) v2 t1) but
